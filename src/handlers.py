...
class MessageHandler:
    ...
    async def handle_message(self, websocket, message: ProtocolMessage) -> ProtocolMessage:
        ...
        try:
            if action == ActionType.CREATE_ROOM.value:
                ...
                return await self.handle_leave_room(websocket, data)
            ...
            log_error(f"Erreur lors du traitement du message: {e}", exc_info=True)
    ...
    async def handle_leave_room(self, websocket, data: Dict[str, Any]) -> ProtocolMessage:
        ...
        if old_room:
            ...
            # Notifier les membres du salon quitté
            await self.broadcast_to_room(
                old_room,
                ProtocolMessage(
                    action=ActionType.RECEIVE_MESSAGE.value,
    ...
    async def broadcast_to_room(self, room_name: str, message: ProtocolMessage, exclude_username: str = None):
        ...
        # Envoyer tous les messages en parallèle
        if send_tasks:
            await asyncio.gather(*send_tasks, return_exceptions=True)
    
    async def send_message(self, websocket, message: ProtocolMessage):
